function spaceChecker(word) {
  // To check for space(s) in a word and replace it with a hyphen e.g. Plastic Bag ---> Plastic-Bag
...
function endDate() {
  // Function to gather all the end days for each month since January 2020 in the format mm/dd/yy
  var dateList = [];
  var cYear = 2020;
  var cMonth = 0;
  var cDay = 1;
      ...
      if (
        ...
        dateList.push(
          `${lastDay.getMonth() + 1}` +
            ...
            `${lastDay.getDate()}` +
            "/" +
            `${lastDay.getYear().toString().substr(-2)}`
        );
        cMonth += 1;
      }
    }
...
mapboxgl.accessToken =
  "pk.eyJ1IjoiazRuZ2dnIiwiYSI6ImNra2NwaG9rMzBneGwyd29sZjQ0ZDlnNW8ifQ.S5yJ6Rta3agYAdE9iNzDmw";
...
$.ajax(byCountries).done(function (response) {
  ...
  for (i = 0; i < response.length; i++) {
    ...
    countryCoordinate.push([
      `${response[i].countryInfo.long}`,
      `${response[i].countryInfo.lat}`,
    ]);
  }

  for (i = 0; i < countryName.length; i++) {
    $("#country-selection").append(
      `<option value=${spaceChecker(countryName[i])}>${countryName[i]}</option>`
    );
  }
  ...
  for (i = 0; i < countryCoordinate.length; i++) {
    ...
    el.addEventListener("click", function (e) {
      ...
      let info = e.target.markerInformation._content.getElementsByClassName(
        "country-name"
      )[0].textContent;
  ...
  $("#country-selection").change(function (e) {
    // If user selected an option from the drop-down menu,
